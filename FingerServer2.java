package com.java21days;         // modified FingerServer app to use the try-with-resources
                                // improvements to the existing try-catch blocks
import java.io.*;
import java.net.*;
    ...
    public FingerServer2() {
        try (ServerSocketChannel sock = ServerSocketChannel.open();
            ...
            InetSocketAddress server = new InetSocketAddress(
                "localhost", 79);
//old       ServerSocket socket = sock.socket();
            socket.bind(server);
...
// old                  Socket connection = sSock.accept();

                            // Handle the Finger request
                            handleRequest(connection);
// old                  connection.close();
                        }   // needed additional closing "}"
                    }
                }
            }
